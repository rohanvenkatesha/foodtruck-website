...
export default function TodaySpecials() {
  return (
    <section id="today" className="py-24 bg-[#FFF8F0] text-center">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }} viewport={{ once: true, amount: 0.3 }}>
        <h2 className="text-4xl md:text-5xl font-bold text-orange-900 mb-4">🔥 Today’s Specials</h2>
        <p className="text-orange-900/70 text-lg max-w-xl mx-auto mb-16">Handpicked dishes made fresh for you today — don’t miss out!</p>
      </motion.div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto px-4">
        {dailySpecials.map((item, index) => (
          <motion.div key={index} className="bg-white rounded-2xl shadow-subtle hover:shadow-hovery transition-shadow duration-300 overflow-hidden flex flex-col group" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * index, duration: 0.5, ease: 'easeOut' }} viewport={{ once: true, amount: 0.2 }}>
<div className="overflow-hidden rounded-t-2xl bg-white flex justify-center items-center" style={{ width: '100%', height: '170px' }}>
  <img
    ...
    onError={(e) => {
      e.currentTarget.src = `https://placehold.co/400x250/FF7F50/FFFFFF?text=${item.name.replace(/\s/g, '+')}`;
